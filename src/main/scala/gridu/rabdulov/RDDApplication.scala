...
import gridu.rabdulov.Model.TopCategoryProducts
...
//      .set("spark.hadoop.mapreduce.input.fileinputformat.input.dir.recursive","true")
    ...
    val networks = networksRDD.map(_.split(",")).map(Utils.TempIP.parse).keyBy(_.geonameId)

    val countriesRDD = sc.textFile("/Users/rabdulov/Downloads/hadoop/rabdulov/GeoLite2-Country-Locations-en.csv")
    val countries = countriesRDD.map(_.split(",")).map(Utils.TempLoc.parse).keyBy(_.geonameId)
    countries.partitionBy(new HashPartitioner(networks.partitions.length))
    countries.join(networks)
    */
